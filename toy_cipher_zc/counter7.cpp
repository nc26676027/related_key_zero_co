...
int encrypt_all( unsigned alpha1 , unsigned alpha2 )
{
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {
                counter[alpha1][alpha2] += unsigned(Encoding12[i][j]);
            }
        }
    }
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {
                counter[alpha1][alpha2] += unsigned(Encoding13[i][j]);
            }
        }
    }
...
int testTK1()
{
    ...
    outfile<<"#ifndef _COUNTERSEVEN_H"<<endl;
    outfile<<"#define _COUNTERSEVEN_H"<<endl;
    outfile<<endl; 
    string group_num = "counterseven";
