...
int encrypt_all( unsigned alpha1 , unsigned alpha2 )
{
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {
                counter[alpha1][alpha2] += unsigned(Encoding6[i][j]);
            }
        }
    }
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {
                counter[alpha1][alpha2] += unsigned(Encoding7[i][j]);
            }
        }
    }
...
int testTK1()
{
    ...
    /*---------------------------打印计数器---------------------------*/
    string file = "counterfour.h";
    ofstream outfile;
    outfile.open(file);

    outfile<<"#include <stdint.h>"<<endl;
    outfile<<"#ifndef _COUNTERFOUR_H"<<endl;
    outfile<<"#define _COUNTERFOUR_H"<<endl;
    outfile<<endl; 
    string group_num = "counterfour";
