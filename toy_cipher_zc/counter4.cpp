...
#include "../table0_h/table6.h"
#include "../table0_h/table7.h"
...
int encrypt_all( unsigned alpha1 , unsigned alpha2 )
{
    ...
    for(int i = 0;i < N1;i++)
    {
        ...
        {
            XOR = 0;
            unsigned p0 = i ^ rol(0x6 , 8); 
            XOR = XOR ^ (alpha1 & p0) ^ (alpha2 & j);
            bool judge = get_xored(XOR);
            
            if( ! judge )
            {
                counter[a1][alpha2] += unsigned(Encoding6[i][j]);
            }
        }
    }
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {                
                counter[a1][alpha2] += unsigned(Encoding7[i][j]);
            }
        }
    }
...
int testTK1(int num)
{
    ...
    /*---------------------------打印计数器---------------------------*/
    string file = "counterfour"+to_string(num)+".h";
    ofstream outfile;
    outfile.open(file);

    outfile<<"#include <stdint.h>"<<endl;
    outfile<<"#ifndef _COUNTERFOUR"<<num<<"_H"<<endl;
    outfile<<"#define _COUNTERFOUR"<<num<<"_H"<<endl;
    outfile<<endl; 
    string group_num = "counterfour"+to_string(num);
