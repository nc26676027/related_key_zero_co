...
int encrypt_all( unsigned alpha1 , unsigned alpha2 )
{
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {
                counter[a1][alpha2] += unsigned(Encoding4[i][j]);
            }
        }
    }
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {                
                counter[a1][alpha2] += unsigned(Encoding5[i][j]);
            }
        }
    }
...
int testTK1(int num)
{
    ...
    outfile<<"#ifndef _COUNTERTHREE"<<num<<"_H"<<endl;
    outfile<<"#define _COUNTERTHREE"<<num<<"_H"<<endl;
    outfile<<endl; 
    string group_num = "counterthree"+to_string(num);
