...
int encrypt_all( unsigned alpha1 , unsigned alpha2 , unsigned beta )
{
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            unsigned p0 = i ^ rol(0x4 , 12 ); 
            XOR = XOR ^ (alpha1 & p0) ^ (alpha2 & j) ^(beta & rol(unsigned(Encoding4[i][j]) , 12) );
        }
    }
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            unsigned p1 = i ^ rol(0x5 , 12 ); 
            XOR = XOR ^ (alpha1 & p1) ^ (alpha2 & j) ^(beta & rol(unsigned(Encoding5[i][j]) , 12) );
        }
    }
