...
#include "../table0_h/table4.h"
#include "../table0_h/table5.h"
...
int encrypt_all( unsigned alpha1 , unsigned alpha2 )
{
    ...
    for(int i = 0;i < N1;i++)
    {
        ...
        {
            XOR = 0;
            unsigned p0 = i ^ rol(0x4 , 8); 
            XOR = XOR ^ (alpha1 & p0) ^ (alpha2 & j);
            bool judge = get_xored(XOR);
            
            if( ! judge )
            {
                counter[alpha1][alpha2] += unsigned(Encoding4[i][j]);
            }
        }
    }
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {
                counter[alpha1][alpha2] += unsigned(Encoding5[i][j]);
            }
        }
    }
...
int testTK1()
{
    ...
    /*---------------------------打印计数器---------------------------*/
    string file = "counterthree.h";
    ofstream outfile;
    outfile.open(file);

    outfile<<"#include <stdint.h>"<<endl;
    outfile<<"#ifndef _COUNTERTHREE_H"<<endl;
    outfile<<"#define _COUNTERTHREE_H"<<endl;
    outfile<<endl; 
    string group_num = "counterthree";
