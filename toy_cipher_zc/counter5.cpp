...
int encrypt_all( unsigned alpha1 , unsigned alpha2 )
{
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {
                counter[alpha1][alpha2] += unsigned(Encoding8[i][j]);
            }
        }
    }
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {
                counter[alpha1][alpha2] += unsigned(Encoding9[i][j]);
            }
        }
    }
...
int testTK1()
{
    ...
    /*---------------------------打印计数器---------------------------*/
    string file = "counterfive.h";
    ofstream outfile;
    outfile.open(file);

    outfile<<"#include <stdint.h>"<<endl;
    outfile<<"#ifndef _COUNTERFIVE_H"<<endl;
    outfile<<"#define _COUNTERFIVE_H"<<endl;
    outfile<<endl; 
    string group_num = "counterfive";
