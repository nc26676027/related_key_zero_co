...
#include "../table0_h/table8.h"
#include "../table0_h/table9.h"
...
int encrypt_all( unsigned alpha1 , unsigned alpha2 )
{
    ...
    for(int i = 0;i < N1;i++)
    {
        ...
        {
            XOR = 0;
            unsigned p0 = i ^ rol(0x8 , 8); 
            XOR = XOR ^ (alpha1 & p0) ^ (alpha2 & j);
            bool judge = get_xored(XOR);
            
            if( ! judge )
            {
                counter[a1][alpha2] += unsigned(Encoding8[i][j]);
            }
        }
    }
    ...
    for(int i = 0;i < N1;i++)
    {
        for(int j = 0;j < N2;j++)
        {
            ...
            if( ! judge )
            {                
                counter[a1][alpha2] += unsigned(Encoding9[i][j]);
            }
        }
    }
...
int testTK1(int num)
{
    ...
    /*---------------------------打印计数器---------------------------*/
    string file = "counterfive"+to_string(num)+".h";
    ofstream outfile;
    outfile.open(file);

    outfile<<"#include <stdint.h>"<<endl;
    outfile<<"#ifndef _COUNTERFIVE"<<num<<"_H"<<endl;
    outfile<<"#define _COUNTERFIVE"<<num<<"_H"<<endl;
    outfile<<endl; 
    string group_num = "counterfive"+to_string(num);
