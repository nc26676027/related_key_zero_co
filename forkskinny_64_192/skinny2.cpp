...
int main(int argc,char * argv[])
{
	...
	for(int round=0;round<y_ROUND;round++)
	{
		for(int pos=0;pos<16;pos++)
		{
			...
			if( pos<8 )
			{
				...
				outcvc<<"ASSERT( RKin3_"<<round+x_ROUND+r0<<"_"<<pos<<" = y_Sout_"<<round<<"_"<<pos<<" );"<<endl;
			}
		...
		for(int pos=0;pos<16;pos++)
		{
			...
			if(round<ROUND-1)
			{
				if(pos<8)
				{
					...
					outcvc<<"ASSERT( Kin3_"<<round+1<<"_"<<pos<<"[2:2] = BVXOR( LPout3_"<<round<<"_"<<P[pos]<<"[0:0] , LPout3_"<<round<<"_"<<P[pos]<<"[3:3] ) );"<<endl;
					outcvc<<"ASSERT( Kin3_"<<round+1<<"_"<<pos<<"[3:3] = LPout3_"<<round<<"_"<<P[pos]<<"[0:0] );"<<endl;			
				}	
				else
				{
	...
	for(int round=0;round<r0;round++)
	{
		for(int pos=0;pos<16;pos++)
		{
			if(pos<8)
			{
				outcvc<<"ASSERT( RKin_"<<x_ROUND+round<<"_"<<pos<<" = 0bin0010 );"<<endl;
				outcvc<<"ASSERT( RKin2_"<<x_ROUND+round<<"_"<<pos<<" = 0bin0010 );"<<endl;
				outcvc<<"ASSERT( RKin3_"<<x_ROUND+round<<"_"<<pos<<" = 0bin0010 );"<<endl;
			}			
		}
	}
